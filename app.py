...
if check_password():
    ...
    # --- 7. DASHBOARD (EVOLUÇÃO + PROJEÇÃO DE GRUPO) ---
        ...
        if not df.empty:
            with st.expander("🔍 Filtros Avançados", expanded=True):
                if ROLE in ['master', 'admin']:
                    ...
                    # Garante que grupos apareçam mesmo sem trades
            ...
            if not df_f.empty:
                # -----------------------------------------------------------
                # 🛡️ PAINEL APEX 150K (EVOLUÇÃO & PROJEÇÃO POR GRUPO)
                # -----------------------------------------------------------
                if sel_grp != "Todos" and ROLE in ['master', 'admin']:
                    st.markdown(f"### 🛡️ Monitor de Evolução: {sel_grp}")
                    ...
                    if not contas_g.empty:
                        ...
                        ref = contas_g.iloc[0]
                        fase = ref['fase']
                        saldo_cadastrado = float(ref['saldo_inicial']) # Saldo Atual na Corretora (Input manual)
                        
                        # Lucro do App (Considerando apenas trades APÓS a data do cadastro ou filtro total)
                        # Nota: Para simplificar, assumimos que o saldo cadastrado já contempla o passado, 
                        # e somamos o resultado do filtro atual para projetar o futuro imediato.
                        lucro_app = df_f['resultado'].sum() 
                        saldo_estimado = saldo_cadastrado + lucro_app # Ajuste conforme sua lógica de atualização
                        
                        # 2. Regras Apex (150k)
                        # Simula curva de equity para High Water Mark
                        ...
                        # 3. Definição de Metas (Target) e Bases (Start)
                        meta_alvo = 0.0
                        base_fase = 150000.0 # Onde começou a fase (apenas para barra de progresso)
                        msg_fase = ""
                        
                        if "Fase 1" in fase: # 150k -> 159k
                            base_fase = 150000.0; meta_alvo = 159000.0; msg_fase = "Teste (Alvo: $159k)"
                        elif "Fase 2" in fase: # 150k -> 155.1k
                            base_fase = 150000.0; meta_alvo = 155100.0; msg_fase = "Colchão (Alvo: $155.1k)"
                        elif "Fase 3" in fase: # 155k -> 160k
                            base_fase = 155100.0; meta_alvo = 160000.0; msg_fase = "Dobra (Alvo: $160k)"
                        elif "Fase 4" in fase: 
                            base_fase = 150100.0; meta_alvo = 0.0; msg_fase = "Renda (Sem Teto)"

                        # 4. Cálculos de Progresso
                        falta_meta = meta_alvo - saldo_estimado
                        
                        # Lógica da % de Conclusão Visual
                        if meta_alvo > 0:
                            total_a_percorrer = meta_alvo - base_fase
                            ja_percorreu = saldo_estimado - base_fase
                            perc_conclusao = min(1.0, max(0.0, ja_percorreu / total_a_percorrer)) if total_a_percorrer > 0 else 0.0
                        else:
                            perc_conclusao = 1.0 # Fase 4 não tem teto

                        # 5. Projeção de Trades (REGRA: MÉDIA DO GRUPO)
                        # Se houver trades neste grupo, usa a média dele. Se não, usa 0 (sem projeção).
                        media_trade_grupo = df_f['resultado'].mean() if not df_f.empty else 0.0

                        # --- VISUALIZAÇÃO ---
                        col_kpi, col_graph = st.columns([1.5, 2.5])
                        
                        with col_kpi:
                            # Métricas Principais
                            st.metric("Saldo Unitário Estimado", f"${saldo_estimado:,.2f}", f"Stop: ${stop_real:,.0f}")
                            
                            cor_b = "inverse" if buffer_vida < 2500 else "normal"
                            st.metric("🔥 Buffer de Vida", f"${buffer_vida:,.2f}", "Margem de Erro", delta_color=cor_b)
                            
                            if meta_alvo > 0:
                                st.write(f"**Progresso da Fase:** {perc_conclusao*100:.1f}%")
                                st.progress(perc_conclusao)
                                
                                if falta_meta > 0 and media_trade_grupo > 0:
                                    trades_restantes = int(falta_meta / media_trade_grupo) + 1
                                    st.info(f"🎯 Faltam **{trades_restantes} trades** na média deste grupo (${media_trade_grupo:.0f}) para passar.")
                                elif falta_meta <= 0:
                                    st.success("🏆 Meta Atingida! Mude a fase na aba Contas.")
                                else:
                                    st.warning("⚠️ Opere mais neste grupo para ter projeção.")
                            else:
                                # Fase 4
                                saque = max(0, saldo_estimado - 152600) * len(contas_g)
                                st.metric("💰 Saque Disp. (Total)", f"${saque:,.2f}", "Acima do Buffer")
                                st.success("🚀 Modo Saque Ativo")

                        with col_graph:
                            # Gráfico de Evolução EXCLUSIVO DO GRUPO
                            # Simula a evolução patrimonial baseada nos trades filtrados
                            df_f_sorted = df_f.sort_values('created_at')
                            df_f_sorted['saldo_historico'] = df_f_sorted['resultado'].cumsum() + saldo_cadastrado
                            
                            fig_evolucao = px.area(df_f_sorted, x='created_at', y='saldo_historico', 
                                                 title=f"Curva de Evolução ({sel_grp})", 
                                                 template="plotly_dark")
                            
                            # Linhas de Referência (Meta e Stop)
                            if meta_alvo > 0:
                                fig_evolucao.add_hline(y=meta_alvo, line_dash="dot", line_color="#00FF88", annotation_text="Meta")
                            fig_evolucao.add_hline(y=stop_real, line_dash="dash", line_color="#FF4B4B", annotation_text="Stop Atual")
                            
                            fig_evolucao.update_traces(line_color='#B20000', fillcolor='rgba(178, 0, 0, 0.1)')
                            st.plotly_chart(fig_evolucao, use_container_width=True)

                    st.markdown("---")

                # -----------------------------------------------------------
                # DASHBOARD GERAL (ABAIXO)
                # -----------------------------------------------------------
                ...
                r1 = st.columns(4)
                with r1[0]: card_metric("RESULTADO (FILTRO)", f"${net:,.2f}", f"Bruto: ${wins['resultado'].sum():,.0f}", "#00FF88" if net>=0 else "#FF4B4B")
                ...
                with g1:
                    st.plotly_chart(px.bar(df_f, x='created_at', y='resultado', title="Resultado por Trade (Cronológico)", template="plotly_dark", color='resultado', color_continuous_scale=["#FF4B4B", "#00FF88"]), use_container_width=True)
                with g2:
                    # Contexto
                    st.plotly_chart(px.pie(df_f, names='contexto', values='lote', title="Volume por Contexto", template="plotly_dark", hole=0.4), use_container_width=True)

            else: st.info("Sem dados. Registre operações ou ajuste os filtros.")
    ...
    # --- 9. ABA CONTAS (COM GESTÃO DE SALDO ATUAL) ---
