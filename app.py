        ...
        if not df_raw.empty:
            ...
            if not df.empty:
                ...
                if df_filtered.empty:
                    ...
                    # ==============================================================================
                    # 🔥 PAINEL INTELIGENTE: GESTÃO DE RISCO APEX 150K
                    # ==============================================================================
                    if sel_grupo != "Todos" and ROLE in ['master', 'admin']:
                        st.markdown(f"### 🛡️ Visão do Grupo: {sel_grupo}")
                        df_contas = load_contas_config()
                        contas_g = df_contas[df_contas['grupo_nome'] == sel_grupo]
                        
                        if not contas_g.empty:
                            # 1. PEGAR DADOS DA CONTA REFERÊNCIA
                            ref = contas_g.iloc[0]
                            fase = ref['fase']
                            saldo_inicial_cadastro = float(ref['saldo_inicial']) # Ex: 150500.0
                            
                            # 2. CALCULAR LUCRO DESDE O INÍCIO DO FILTRO
                            # (Idealmente seria desde o cadastro, mas vamos usar o lucro filtrado no dashboard para dar dinamismo)
                            lucro_periodo = df_filtered['resultado'].sum()
                            
                            # 3. SALDO ATUAL ESTIMADO (Por Conta)
                            saldo_atual_est = saldo_inicial_cadastro + lucro_periodo
                            
                            # 4. CÁLCULO DO TRAILING STOP (APEX 150K RULES)
                            # O Trailing sobe com o lucro MÁXIMO atingido (High Water Mark).
                            # Vamos calcular o HWM com base na curva de equity deste período.
                            
                            # Equity curve partindo do saldo inicial
                            equity_curve = df_filtered.sort_values('created_at')['resultado'].cumsum() + saldo_inicial_cadastro
                            
                            # Se não tem trades, HWM é o saldo inicial
                            if equity_curve.empty:
                                high_water_mark = saldo_inicial_cadastro
                            else:
                                high_water_mark = max(saldo_inicial_cadastro, equity_curve.max())
                            
                            # Regra: Stop é HWM - 5000, mas trava em 150.100
                            trailing_stop_teorico = high_water_mark - 5000
                            
                            # TRAVAMENTO DO STOP
                            stop_loss_real = min(trailing_stop_teorico, 150100.0)
                            
                            # Se o saldo inicial já estava acima de 155.100, o stop já estava travado em 150.100
                            if saldo_inicial_cadastro > 155100:
                                stop_loss_real = 150100.0
                            elif trailing_stop_teorico > 150100:
                                stop_loss_real = 150100.0

                            # 5. BUFFER DE VIDA (O NÚMERO MAIS IMPORTANTE)
                            buffer_vida = saldo_atual_est - stop_loss_real
                            
                            # 6. DEFINIÇÃO DE METAS POR FASE
                            meta_alvo = 0.0
                            ...
                            k1.metric("Contas Replicadas", f"{len(contas_g)}", f"Alavancagem: {len(contas_g)}x")
                            k2.metric("Saldo Unitário (Est.)", f"${saldo_atual_est:,.2f}", f"Stop em: ${stop_loss_real:,.0f}")
                            
                            # Cor do Buffer
                            cor_b = "inverse" if buffer_vida < 2500 else "normal" # Alerta se < 2.5k (metade)
                            k3.metric("🔥 Buffer de Vida", f"${buffer_vida:,.2f}", "Até quebrar", delta_color=cor_b)
                            ...
                            if meta_alvo > 0 and falta_meta > 0:
                                ...
                                # Modo Saque: Buffer de segurança de 2500, o resto é saque
                                saque_disp = max(0, saldo_atual_est - 152600) * len(contas_g)
                                k4.metric("💰 Saque Disponível", f"${saque_disp:,.2f}", "Total do Grupo")

                            # --- CÉREBRO: PROJEÇÃO NEURO-FRIENDLY ---
                            st.write("")
                            c_mind, c_risk = st.columns([2, 1])
                            
                            with c_mind:
                                if meta_alvo > 0 and falta_meta > 0 and media_trade > 0:
                                    trades_nec = int(falta_meta / media_trade) + 1
                                    st.info(f"🧠 **Foco na Execução:** Faltam apenas **{trades_nec} trades bem feitos** (na sua média de ${media_trade:.0f}) para bater a meta.")
                                elif meta_alvo > 0 and falta_meta <= 0:
                                    st.success("🏆 **PARABÉNS!** Meta atingida. Solicite a aprovação/upgrade.")
                                elif "Fase 4" in fase:
                                    st.success("🚀 **Fase de Renda:** Mantenha o buffer acima de $3.000 e opere tranquilo.")
                                else:
                                    st.warning("⚠️ Precisa de média positiva para projetar trades.")

                            with c_risk:
                                # Barra de Risco: Quanto do meu drawdown de 5k eu já comi?
                                # Drawdown Total Possível = 5000 (ou menos se travou)
                                # Buffer Atual = X
                                # Risco Usado = 1 - (Buffer / 5000)
                                risco_usado = 1.0 - (buffer_vida / 5000.0)
                                risco_usado = min(1.0, max(0.0, risco_usado))
                                st.write(f"**Risco Consumido: {risco_usado*100:.1f}%**")
                                st.progress(risco_usado)
                    ...
                    st.markdown("##### 🏁 Desempenho Global (Filtro)")
            ...
            with tab_conta:
                ...
                if df_g.empty:
                    ...
                    with st.form("form_conta"):
                        ...
                        # MUDANÇA: Saldo Atual em vez de Saldo Inicial
                    ...
                    for grp in grupos_unicos:
                        with st.expander(f"📂 {grp}", expanded=True):
                            ...
                            for _, row in contas_g.iterrows():
                                # Lógica: Saldo Atual = Saldo Cadastrado + Lucro
