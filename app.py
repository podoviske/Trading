...
if check_password():
    ...
    if selected == "Dashboard":
        ...
        if not df.empty:
            ...
            if not df_f.empty:
                # -----------------------------------------------------------
                # 1. PAINEL APEX 150K (GESTÃO DE RISCO) - TOPO
                # -----------------------------------------------------------
                if sel_grp != "Todos" and ROLE in ['master', 'admin']:
                    ...
                    if not contas_g.empty:
                        ...
                        k2.metric("Saldo Unitário", f"${saldo_atual_est:,.2f}", f"App: ${lucro_periodo:+,.2f}")
                ...
                with g2:
                    ...
                    # Gráfico de Contexto restaurado
    ...
    # --- 8. REGISTRAR TRADE (COM CONTEXTO E PSICOLOGIA) ---
        ...
        with f1:
            ...
            ctx = st.selectbox("Contexto Técnico", ["Tendência", "Lateralidade", "Rompimento", "Contra-Tendência", "Pullback", "Outro"])
            # PSICOLOGIA (MANTIDO)
