...
# --- CONEXÃO SUPABASE ---
# As chaves devem ser configuradas no Streamlit Cloud (Secrets) ou localmente (.streamlit/secrets.toml)
try:
...
def check_password():
    def password_entered():
        ...
        try:
            # Busca na tabela 'users' que criamos no Supabase
...
if check_password():
    ...
    # --- FUNÇÕES DE CARREGAMENTO (DATABASE) ---
    def load_trades_db():
    ...
    with st.sidebar:
        ...
        selected = option_menu(None, menu, icons=["grid", "currency-dollar", "gear", "clock"], styles={"nav-link-selected": {"background-color": "#B20000"}})
    ...
    if selected == "Registrar Trade":
        ...
        with f3:
            ...
            saidas = []
            aloc = 0
            for i, p_c in enumerate(parciais_list):
                c_pts, c_qtd = st.columns(2)
                pts = c_pts.number_input(f"Pts P{i+1}", value=float(p_c["pts"]), key=f"pts_{i}")
                qtd = c_qtd.number_input(f"Qtd P{i+1}", value=int(p_c["qtd"]), key=f"qtd_{i}")
        ...
        if st.button("💾 REGISTRAR TRADE", use_container_width=True) and lt == aloc:
            with st.spinner("Salvando dados..."):
                ...
                # Upload da imagem para o Bucket 'prints'
                img_url = ""
                if up:
                ...
                supabase.table("trades").insert({
                    ...
                    "print_url": img_url, "usuario": st.session_state["logged_user"]
                }).execute()
        ...
        if not df.empty:
            ...
            for _, row in df_h.iterrows():
                with st.container():
                    ...
                    if row['print_url']:
                        col1.image(row['print_url'], use_container_width=True)
                    else:
                        col1.write("Sem Imagem")
                    col2.write(f"**{row['data']} - {row['ativo']} ({row['contexto']})**")
                    col2.write(f"Direção: {row['direcao']} | Lote: {row['lote']} | Pts: {row['pts_medio']:.1f}")
