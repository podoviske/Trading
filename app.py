...
if check_password():
            ...
            if not df_f.empty:
                ...
                if sel_grp != "Todos" and ROLE in ['master', 'admin']:
                    ...
                    if not contas_g.empty:
                        ...
                        ts = hwm - 5000.0
                        stop_real = 150100.0 if (saldo_cadastrado > 155100 or ts > 150100) else min(ts, 150100.0)
                        ...
                        if meta_alvo > 0: k4.metric("Falta", f"${meta_alvo-saldo_atual_est:,.2f}", f"Alvo: ${meta_alvo:,.0f}")
                        else: k4.metric("Saque", f"${max(0, saldo_atual_est-152600)*len(contas_g):,.2f}")
                    st.markdown("---")
                ...
                with r1[1]: card_metric("FATOR DE LUCRO", f"{pf:.2f}", "", "#B20000")
    ...
    # ==============================================================================
        ...
        with c_grp:
            ...
            if ROLE in ["master", "admin"]:
                if not df_grupos.empty: grupo_sel_trade = st.selectbox("📂 Grupo", sorted(df_grupos['nome'].unique()))
    ...
    # ==============================================================================
    # 9. ABA CONTAS (MONITOR APEX COM TRAILING DINÂMICO E FILTRO DE VISUALIZAÇÃO)
    # ==============================================================================
        ...
        if ROLE not in ['master', 'admin']: st.error("Restrito.")
            ...
            # --- MONITOR DE PERFORMANCE (VERSÃO 117 - FINAL) ---
            with t4:
                ...
                if not df_c.empty:
                    ...
                    if not contas_g.empty:
                        ...
                        # Cálculo de Métricas Básicas (Instantâneo)
                        if not trades_g.empty:
                        ...
                        with cg:
                            ...
                            if not trades_g.empty:
                                ...
                                # Adiciona Ponto Inicial (Zero Trade)
                                ...
                                df_plot = pd.DataFrame([{'eixo_x': datetime.now(), 'saldo_acc': saldo_base}])

                            # --- CÁLCULO DO TRAILING STOP LINHA A LINHA ---
                            df_plot['hwm'] = df_plot['saldo_acc'].cummax()
                            
                            # Lógica Apex: Stop = HWM - 5000, mas nunca maior que 150.100 (exceto se começou acima)
                            limit_stop = 150100.0
                            ...
                            # Zoom Automático
                            min_y = min(df_plot['stop_line'].min(), df_plot['saldo_acc'].min()) - 500
                            max_y = max(meta if meta > 0 else saldo_atual, df_plot['saldo_acc'].max()) + 500
