...
# --- ESTILO CSS (Incluindo Janelas de Prédio) ---
st.markdown("""
    <style>
    [data-testid="stSidebar"] { background-color: #111111 !important; border-right: 1px solid #1E1E1E; }
    .stApp { background-color: #0F0F0F; }
    
    /* Moldura da Janela do Histórico (Efeito Prédio) */
    .trade-window {
...
# --- DASHBOARD --- (Sua versão recuperada)
if selected == "Dashboard":
    ...
    if not df.empty:
        ...
        df_filtrado = df.copy()
        if filtro_view != "Capital": df_filtrado = df[df['Contexto'] == filtro_view]
...
# --- REGISTRAR TRADE --- (Sua versão recuperada + Campo de Print)
elif selected == "Registrar Trade":
    st.title("Registro de Trade")
    c_topo1, c_topo2 = st.columns([3, 1])
    with c_topo1:
        atm_sel_nome = st.selectbox("🎯 Estratégia ATM", options=list(atm_db.keys()), key='atm_selecionado')
    ...
    with c2:
        ...
        up_files = st.file_uploader("📸 Anexar Prints (Upload ou Ctrl+V)", accept_multiple_files=True)
    ...
    if st.button("💾 REGISTRAR TRADE", use_container_width=True):
        if lote_t > 0 and alocado == lote_t:
            ...
            n_t = pd.DataFrame([{'Data': data, 'Ativo': ativo, 'Contexto': contexto, 'Direcao': direcao, 'Lote': lote_t, 'ATM': atm_sel_nome, 'Resultado': res, 'Pts_Medio': pts_m, 'Risco_Fin': risco, 'ID': t_id, 'Prints': "|".join(paths)}])
            df = pd.concat([df, n_t], ignore_index=True); df.to_csv(CSV_FILE, index=False)
            st.success("🎯 Trade registrado!"); time.sleep(1); st.rerun()

# --- CONFIGURAR ATM --- (Sua versão recuperada)
...
# --- HISTÓRICO --- (Versão Galeria Estilo Janela)
