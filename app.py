...
# Esta parte é o "motor" que permite que o Streamlit salve arquivos permanentemente.
...
if check_password():
    ...
    def load_atm():
        ...
        if atm_bytes:
            try: return json.loads(atm_bytes.decode('utf-8'))
            except: pass
    ...
    def get_base64_cloud(file_name):
        # Adaptado para pegar a imagem do Drive
        try:
            name = file_name.split("/")[-1] if "/" in file_name else file_name
            content = load_from_drive(name)
            ...
            if st.button("💾 REGISTRAR GAIN", use_container_width=True):
                if lote_t > 0 and alocado == lote_t:
                    ...
                    for i, f in enumerate(up_files):
                        p_name = f"{n_id}_{i}.png"; paths.append(p_name)
                ...
                for j in range(5):
                    if i + j < len(df_disp):
                        ...
                        with cols[j]:
                            ...
                            img_html = f'<div class="img-container"><img src="data:image/png;base64,{img_b64}"></div>' if img_b64 else '<div class="img-container" style="color:#444">Sem Print</div>'
