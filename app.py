...
if check_password():
    ...
    st.markdown("""
        ...
        .sidebar-brand-container { display: flex; align-items: center; gap: 12px; padding: 10px 0px 25px 0px; }
    ...
    # --- ABA: DASHBOARD (MÉTRICAS COMPLETAS 3x3) ---
    if selected == "Dashboard":
        st.title("📊 Analytics")
        df_v = df if st.session_state["logged_user"] == "admin" else df[df['Usuario'] == st.session_state["logged_user"]]
        if not df_v.empty:
            ...
            if not df_f.empty:
                ...
                with r1: 
                    card_metric("P&L TOTAL", f"${total_pl:,.2f}", "#00FF88" if total_pl > 0 else "#FF4B4B")
                    card_metric("RISCO:RETORNO", f"1:{rr:.2f}")
                    card_metric("PTS MÉD GAIN", f"{avg_pts_gain:.2f}")
                with r2: 
                    card_metric("WIN RATE", f"{wr:.1f}%", "#B20000")
                    card_metric("LOTE MÉDIO", f"{avg_lote:.1f}")
                    card_metric("PTS MÉD LOSS", f"{avg_pts_loss:.2f}")
                with r3: 
                    card_metric("TRADES TOTAL", total)
                    card_metric("GAIN MÉDIO ($)", f"${aw:,.2f}", "#00FF88")
                    card_metric("LOSS MÉDIO ($)", f"$-{al:,.2f}", "#FF4B4B")
    ...
    # --- ABA: REGISTRAR TRADE (BOTÃO EXTRA + CÁLCULOS RESTAURADOS) ---
