...
def show(user, role):
    ...
    # Inicializa variáveis para não quebrar a tela (Correção NameError)
    ...
    if not df_raw.empty:
        ...
        if not df.empty:
            # --- MOTOR APEX (Calcula Saúde do Grupo) ---
            ...
            # --- CARDS VISUAIS (LINHA 1: GERAL) ---
            st.markdown("##### 🏁 Desempenho Geral")
            c1, c2, c3, c4 = st.columns(4)
            ui.card_metric("RESULTADO", f"${net:,.2f}", color="#00FF88" if net>=0 else "#FF4B4B")
            ui.card_metric("FATOR DE LUCRO", pf_str, "Ideal > 1.5", "#B20000")
            ui.card_metric("WIN RATE", f"{win_rate_dec*100:.1f}%", f"{len(wins)}W / {len(losses)}L")
            ui.card_metric("EXPECTATIVA", f"${expectancy:.2f}", "Por Trade", "#00FF88" if expectancy>0 else "#FF4B4B")
            
            # --- CARDS VISUAIS (LINHA 2: MÉDIAS) ---
            st.markdown("##### 💲 Médias Financeiras")
            c5, c6, c7, c8 = st.columns(4)
            ui.card_metric("MÉDIA GAIN", f"${avg_win:,.2f}", "", "#00FF88")
            ui.card_metric("MÉDIA LOSS", f"-${avg_loss:,.2f}", "", "#FF4B4B")
            ui.card_metric("PAYOFF", f"1 : {payoff:.2f}", "Risco/Retorno")
            df_sorted = df.sort_values('created_at')
            df_sorted['equity'] = df_sorted['resultado'].cumsum()
            max_dd = (df_sorted['equity'] - df_sorted['equity'].cummax()).min()
            ui.card_metric("DRAWDOWN MAX", f"${max_dd:,.2f}", "Pior Queda", "#FF4B4B")

            # --- CARDS VISUAIS (LINHA 3: TÉCNICA) ---
            st.markdown("##### 🎯 Performance Técnica")
            c9, c10, c11, c12 = st.columns(4)
            ui.card_metric("PTS MÉDIOS (GAIN)", f"{avg_pts_gain:.2f} pts", "", "#00FF88")
            ui.card_metric("STOP MÉDIO", f"{pts_loss_medio_real:.2f} pts", "Base Risco", "#FF4B4B")
            ui.card_metric("LOTE MÉDIO", f"{lote_medio_real:.1f}", "Contratos")
            ui.card_metric("TOTAL TRADES", str(total_trades), "Executados")

            # --- RISCO E SOBREVIVÊNCIA (LINHA 4) ---
            st.markdown("---")
            ...
            # Kelly Simplificado para Visualização
            ...
            if risco_base == 0: risco_base = 300.0 * fator_rep # Fallback
            
            vidas = buffer_total / risco_base if risco_base > 0 else 0
            
            k1, k2, k3, k4 = st.columns(4)
            z_score = (win_rate_dec * payoff) - loss_rate_dec
            ui.card_metric("Z-SCORE", f"{z_score:.2f}", "Força do Edge", "#00FF88" if z_score > 0 else "#FF4B4B")
            ui.card_metric("BUFFER REAL", f"${buffer_total:,.0f}", f"{n_contas} Contas", "#00FF88")
            ui.card_metric("VIDAS (U)", f"{vidas:.1f}", f"Base ${risco_base:.0f}", "#FF4B4B" if vidas<6 else "#00FF88")
            
            with k4:
                cor_r = "#FF4B4B" if prob_ruina > 5 else "#00FF88"
                st.markdown(f"""<div style="background:#161616; border:2px solid {cor_r}; border-radius:12px; padding:10px; text-align:center; height:90px; display:flex; flex-direction:column; justify-content:center;">
            ...
            ui.card_metric("BUFFER DISP.", f"${buffer_total:,.0f}", f"Stop Global: ${stop_atual_val:,.0f}", "#00FF88")
            ui.card_metric("HALF-KELLY", f"{kelly_half*100:.1f}%", "Teto Matemático", "#888")
            ui.card_metric("RISCO FIN.", f"${r_min_show:,.0f}", "Sugerido", cor_k)
            with kd:
                st.markdown(f"""<div style="background:#161616; border:2px solid {cor_k}; border-radius:12px; padding:10px; text-align:center; height:90px;">
            ...
            # --- GRÁFICO SEMANAL (CORRIGIDO) ---
            st.markdown("### 📅 Performance por Dia")
            df['data_dt'] = pd.to_datetime(df['data'])
            df['dia_num'] = df['data_dt'].dt.dayofweek # 0=Seg
